...
class taiLieu(Base):
    ...
    cau_hois = relationship("cauHoi", back_populates="tai_lieu", cascade="all, delete-orphan")
...
class cauHoi(Base):
    ...
    # --- FIX: cascade để xóa dapAn con khi xóa cauHoi ---
    dap_ans  = relationship("dapAn", back_populates="cau_hoi", cascade="all, delete-orphan")
...
class baiKiemTra(Base):
    ...
    lich_su_lam_kts   = relationship("lichSuLamKT",    cascade="all, delete-orphan", foreign_keys="lichSuLamKT.id_baiKiemTra")
    chi_tiet_kiem_tras = relationship("chiTietKiemTra", cascade="all, delete-orphan", foreign_keys="chiTietKiemTra.id_baiKiemTra")
...
class chiTietKiemTra(Base):
    ...
    id_lsIKT      = Column(Integer, ForeignKey("lichSuLamKT.id_lsIKT"), nullable=True)
...
class cauTraLoi(Base):
    ...
    noiDungTuLuan = Column(Text, nullable=True)
...
class lichSuLamKT(Base):
    ...
    cau_tra_lois       = relationship("cauTraLoi",    cascade="all, delete-orphan", foreign_keys="cauTraLoi.id_lsIKT")
    chi_tiet_kiem_tras = relationship("chiTietKiemTra", cascade="all, delete-orphan", foreign_keys="chiTietKiemTra.id_lsIKT")
...
class kyNang(Base):
    ...
    tien_dos = relationship("tienDoKyNang", cascade="all, delete-orphan", foreign_keys="tienDoKyNang.id_kyNang")
...
class loTrinh(Base):
    ...
    buoc_hocs = relationship("buocHoc", back_populates="lo_trinh", cascade="all, delete-orphan")
...
class aiGoiY(Base):
    ...
    ngayTao     = Column(DateTime, default=datetime.utcnow)
    diemTinCay  = Column(Integer, default=50)
