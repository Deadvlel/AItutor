...
{{
  "cau_hoi": [
    {{
      ...
      "dap_an_mau": "Đáp án đầy đủ để AI chấm so sánh",
...
def xu_ly_excel(file_bytes: bytes, ten_file: str, db: Session) -> dict:
    ...
    df = (
        pd.read_csv(io.BytesIO(file_bytes), encoding="utf-8")
    ...
    for _, row in df.iterrows():
        ...
        if not noi_dung or noi_dung == "nan":
            continue
        ...
        db.add(cauHoi(
            ...
            dapAnMau     =dap_an,
            loiGiaiThich =dap_an,
            goiY         =goi_y if goi_y and goi_y != "nan" else None,
            thuTu        =thu_tu,
        ))
        da_them += 1
